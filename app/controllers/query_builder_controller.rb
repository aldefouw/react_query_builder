class QueryBuilderController < ApplicationController
	...
	def new
		QueryBuilder.report_included?(params[:query_type]) ?
			...
			redirect_to(query_builder_index_path)
	end
	...
	def save_field_mappings
		...
		redirect_to params[:action] == "update" ?
			            edit_query_builder_path(id: params[:id]) :
			            new_query_builder_path(query_type: params[:query_type])
	end
	...
	def save_as_query_criteria
		params[:save_query] && @query_form.validate(params[:save_query]) && params[:commit] != "Save  "
	end
	...
	def config_report(options: set_params,
		...
		if params[:id] && @query.nil?
			return redirect_to query_builder_index_path
		else
	...
	def form_path
		params[:id] ?
			{ url: query_builder_path(id: params[:id]), html: { method: :patch } } :
			{ url: query_builder_index_path, html: { method: :post }  }
	end
