...
RSpec.describe ReactQueryBuilder::QueryBuilderController, type: :controller do
	...
	describe "create" do
		...
		it 'should save Field Mappings to the QB Field Mapping table if they are present' do
			expect(ReactQueryBuilder::QbFieldMapping.find_by(model: "QbPerson").labels).to eq({"account_timeout"=>"Account Timeout", "active"=>"Active", "comp_rate" =>"Comp Rate", "first_name"=>"First Name", "hire_date" => "Hire Date", "id"=>"Id", "last_name"=>"Last Name", "middle_name"=>"Middle Name", "status"=>"Status", "trained"=>"Trained", "username"=>"Username"})
